...
@pytest.mark.parametrize("dim", [2, 3])
@pytest.mark.parametrize("P", [1, 2, 3, 4, 5])
def test_matrix_surface_kernel(dim, kernel_type, P):
    N = 30 if dim == 2 else 10
    mesh = dolfinx.UnitSquareMesh(MPI.COMM_WORLD, N, N) if dim == 2 else dolfinx.UnitCubeMesh(MPI.COMM_WORLD, N, N, N)
    ...
    kernel = dolfinx_cuas.cpp.contact.generate_jacobian_kernel(V._cpp_object, kernel_type, 2 * P + 1, [])
    B.zeroEntries()
    ...
    compare_matrices(A, B, atol = 1e-11)
