...
from dolfinx_cuas.verification import compute_reference_stiffness_matrix
...
if __name__ == "__main__":
    ...
    for i in range(runs):
        start = time.time()
        Aref = compute_reference_stiffness_matrix(V, quadrature_degree, jit_options)
        end = time.time()
        print(f"{i}: DOLFINx {end-start:.2e}")
        if i > 0:
            dolfin_times[i - 1] = end - start
        start = time.time()
        A = assemble_matrix_numba(V, quadrature_degree, "stiffness")
        end = time.time()
        if i > 0:
