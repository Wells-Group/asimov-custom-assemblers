    ...
    std::vector<std::shared_ptr<const dolfinx::fem::Function<PetscScalar>>> coeffs, int Q)
{
  ...
  //   xt::xtensor<double, 2> coord = xt::adapt(coordinate_dofs, gdim * d, xt::no_ownership(),
  //   shape);
  ...
  // // Tr(eps(u))I:eps(v) dx
  // //========================================================================================
  // kernel_fn tr_eps = [=](double* A, const double* c, const double* w, const double*
  // coordinate_dofs,
  //                        const int* entity_local_index, const std::uint8_t*
