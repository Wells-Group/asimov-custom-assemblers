...
public:
  ...
  void create_facet_normals(int origin_meshtag)
  {
    // Mesh info
    auto mesh = _marker->mesh();
    ...
    // FIXME: There must be an easier way to get this information
    auto num_loc_facets = basix::cell::topology(basix_element.cell_type())[tdim - 1]
                              .size(); // Topology of basix facets
    auto num_quadrature_pts = _qp_ref_facet.shape(1);
    // Structure needed for jacobian of cell basis function
    xt::xtensor<double, 4> dphi_c({num_loc_facets, tdim, num_quadrature_pts, basix_element.dim()});
    for (int i = 0; i < num_loc_facets; ++i)
    {
    ...
    xt::xtensor<double, 2>* normals_pt; // pointer to point to _normals_"origin_meshtag"

    if (origin_meshtag == 0)
    {
      facets = &_facet_0;
      _normals_0 = xt::zeros<double>({(*facets).size(), size_t(gdim)});
      normals_pt = &_normals_0;
    }
    else
    {
      facets = &_facet_1;
      _normals_1 = xt::zeros<double>({(*facets).size(), size_t(gdim)});
      normals_pt = &_normals_1;
    }
    size_t num_facets = (*facets).size();
    for (int i = 0; i < num_facets; i++)
    {
      int facet = (*facets)[i]; // extract facet
      auto cells = f_to_c->links(facet);
      // since the facet is on the boundary it should only link to one cell
      assert(cells.size() == 1);
      auto cell = cells[0]; // extract cell
      ...
      const std::int32_t facet_index = std::distance(cell_facets.data(), local_facet);
      ...
      dolfinx_cuas::math::compute_jacobian(dphi0_c, coordinate_dofs, J);
      dolfinx_cuas::math::compute_inv(J, K);

      auto n_phys = xt::view((*normals_pt), i, xt::all());
      auto facet_normal = xt::row(facet_normals, facet_index);
      for (std::size_t k = 0; k < gdim; k++)
        for (std::size_t j = 0; j < tdim; j++)
          n_phys(k) += K(j, k) * facet_normal[j];
  ...
  /// Pack normals on opposite surface corresponding to quadrature points on
  /// surface_"origin_meshtag"
  /// @param[in] orgin_meshtag - surface on which to integrate
  /// @param[out] c - normals packed on facets. c[i, gdim * k+ j] contains the jth component of the
  /// normal on the ith facet at kth quadrature point
  dolfinx::array2d<PetscScalar> pack_normals(int origin_meshtag)
  {
    ...
    std::vector<int32_t>* candidate_facets;
    std::shared_ptr<dolfinx::graph::AdjacencyList<std::int32_t>> map;
    xt::xtensor<double, 2>* normals_pt;
    if (origin_meshtag == 0)
    {
      puppet_facets = &_facet_0;
      map = _map_0_to_1;
      create_facet_normals(1);
      normals_pt = &_normals_1;
      candidate_facets = &_facet_1;
    }
    else
    ...
    for (int i = 0; i < num_facets; ++i)
    {
      ...
      {
        auto facet = links(j);
        auto facet_it = std::find((*candidate_facets).begin(), (*candidate_facets).end(), facet);
        const std::int32_t facet_index = std::distance((*candidate_facets).begin(), facet_it);
        for (int k = 0; k < gdim; k++)
        {
          row[j * gdim + k] += (*normals_pt)(facet_index, k);
        }
      }
    }
    return c;
